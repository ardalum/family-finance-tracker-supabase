      ...
      {rows.length === 0 ? <Empty message={emptyMessage} /> : (
        <div className="grid gap-3 p-4">
          {previewRows.map((row) => {
            ...
            return (
              <article key={row.card.id} className="rounded-2xl border border-app-border bg-app-surface px-4 py-3">
                <div className="flex min-w-0 flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-xs text-text-muted">
                      {row.card.owner || "No owner"} · {row.card.bank || "No bank"} · Due day {row.card.dueDay}
                    </p>
                  </div>
                ...
                <div className="mt-3 grid gap-2 rounded-xl bg-app-background px-3 py-2 text-xs text-text-muted sm:grid-cols-2">
                  <p>
                    Days until due: <span className="font-semibold text-text-main">{row.daysUntilDue}</span>
                  </p>
                  <p>
                    Paid status: <span className="font-semibold text-text-main">{row.paid ? "Paid" : "Not paid"}</span>
                  </p>
                </div>
              </article>
            );
          ...
          {hiddenCount > 0 ? (
            <p className="px-1 text-xs font-medium text-text-muted">
              Showing {previewRows.length} of {rows.length}. Open Credit Cards to review all balances.
            </p>
          ) : (
            <p className="px-1 text-xs font-medium text-text-muted">
