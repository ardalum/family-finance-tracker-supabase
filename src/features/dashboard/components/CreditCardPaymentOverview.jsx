    ...
    <Card className="overflow-hidden">
      <div className="border-b border-app-border p-5">
        ...
        <p className="mt-1 text-xs text-text-muted">
          {dueSoonCount} card{dueSoonCount === 1 ? "" : "s"} due within 7 days
        </p>
      </div>
      {rows.length === 0 ? (
            ...
            return (
              ...
              >
                <div className="mb-1 flex items-center justify-between gap-2">
                  ...
                  <span
                    className={`rounded-full px-2 py-0.5 text-xs font-semibold ${pastDue ? "bg-status-dangerBg text-status-dangerDark" : dueSoon ? "bg-status-warningBg text-status-warningDark" : row.paid ? "bg-status-successBg text-status-successDark" : "bg-app-muted text-text-soft"}`}
                  >
                ...
                <div className="flex items-center justify-between text-xs text-text-muted">
                  <span>
                    Due in {row.daysUntilDue} day{Math.abs(row.daysUntilDue) === 1 ? "" : "s"}
                  </span>
