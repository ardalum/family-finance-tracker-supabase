      ...
      {transactions.length === 0 ? (
        ...
        <div className="grid gap-3 p-4">
          {previewTransactions.map((transaction) => {
            ...
            const categoryLabel = transaction.splits.map((split) => getCategoryName(split.categoryId, categories)).join(", ");
            return (
              <article key={transaction.id} className="rounded-2xl border border-app-border bg-app-surface px-4 py-3">
                <div className="flex min-w-0 flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-xs text-text-muted">
                      ...
                      {card ? <> · <LinkedCardName card={card} /></> : transaction.cardId ? ` · ${getCardName(transaction.cardId, cards)}` : ""}
                    </p>
                  </div>
                  <p className="text-sm font-semibold text-text-main">{formatCurrency(transaction.amount)}</p>
                </div>
              </article>
            );
          })}
