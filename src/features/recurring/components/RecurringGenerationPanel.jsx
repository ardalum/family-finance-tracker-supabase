...
export default function RecurringGenerationPanel({
  ...
  const workflowCounts = useMemo(
    () => ({
      ...
      needsAction: billRows.filter((row) => !["paid", "skipped"].includes(row.instance?.status)).length,
  ...
  async function handleMarkPaid(row) {
    ...
    if (!Number.isFinite(amount) || amount <= 0) {
      setMessage({ type: "error", text: "Enter an actual amount greater than zero before marking paid." });
      return;
    }
        ...
        <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-start">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">
              Review each bill, enter the actual amount, then mark it paid. Paid bills create or update one linked spending transaction.
            </p>
          </div>
          <div className="grid grid-cols-2 gap-2 text-xs sm:grid-cols-4 lg:min-w-[420px]">
            ...
            <StatusCount label="Past due" value={workflowCounts.pastDue} tone={workflowCounts.pastDue > 0 ? "danger" : "default"} />
          </div>
        </div>

        <div className="rounded-xl border border-blue-100 bg-blue-50 px-3 py-2 text-xs text-blue-800">
          Tip: Use <span className="font-semibold">Update Paid</span> to correct the amount or paid date. Use <span className="font-semibold">Mark Unpaid</span> to remove the linked spending transaction for this month.
        </div>

        {message ? (
          <p className={`rounded-md border px-3 py-2 text-sm ${messageStyles[message.type] ?? messageStyles.success}`}>
            {message.text}
          </p>
        ) : null}
      </div>
          ...
          <table className="min-w-full border-separate border-spacing-0 text-left text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {billRows.map((row) => {
                ...
                return (
                  <tr key={row.template.id} className={row.displayStatus === "Past due" ? "bg-red-50/70" : "bg-white"}>
                    <td className="px-5 py-4 align-middle font-semibold text-gray-950">
                      <div className="grid gap-1">
                        <span>{row.template.name}</span>
                        {isPaid ? (
                          <span className="text-xs font-medium text-gray-500">Linked spending transaction protected from direct edit/delete.</span>
                        ) : needsAction ? (
                          <span className="text-xs font-medium text-gray-500">Waiting for amount and paid date.</span>
                        ) : null}
                      </div>
                    </td>
                    ...
                    <td className="px-5 py-4 align-middle">
                      <span className={`inline-flex items-center gap-1.5 rounded-md px-2.5 py-1 text-xs font-semibold ring-1 ring-inset ${statusStyles[row.displayStatus] ?? statusStyles.Upcoming}`}>
                        ...
                        {!isPaid && !isSkipped && row.displayStatus !== "Past due" ? <Clock size={13} aria-hidden="true" /> : null}
                        {row.displayStatus}
                      </span>
                    </td>
                    <td className="px-5 py-4 align-middle">
                    ...
                    <td className="px-5 py-4 align-middle">
                      ...
                      {isPaid ? (
                        ...
                        <p className="mt-2 text-xs text-gray-500">Skipped bills do not create spending transactions.</p>
                      ) : null}
                    </td>
                  </tr>
                );
...
function StatusCount({ label, value, tone = "default" }) {
  return (
    <div className={`rounded-xl border px-3 py-2 ${tone === "danger" ? "border-red-200 bg-red-50 text-red-700" : "border-gray-200 bg-gray-50 text-gray-600"}`}>
